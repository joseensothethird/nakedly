...
// Add these new styled components to your existing file
...
export const MessageBubble = styled.div`
  ...
  > div {
    display: flex;
    flex-direction: column;
    background: ${props => props.currentUser ? '#3b82f6' : '#fff'};
    color: ${props => props.currentUser ? '#fff' : '#333'};
    padding: 10px 15px;
    border-radius: ${props => 
      props.currentUser 
        ? '18px 18px 0 18px' 
  ...
  ${MessageUserName} {
    font-size: 0.8rem;
    color: ${props => props.currentUser ? '#fff' : '#6c757d'};
    margin-bottom: 2px;
  }

  ${MessageTimestamp} {
    font-size: 0.7rem;
    color: ${props => props.currentUser ? 'rgba(255,255,255,0.7)' : '#6c757d'};
    align-self: flex-end;
    margin-top: 3px;
  }
`;
...
export const ProfileUserHandle = styled.p`
  margin: 0.25rem 0;
  color: #666;
`;
...
export const SubscribePrompt = styled.button`
  background: none;
  border: none;
  color: #3a86ff;
  font-weight: bold;
